...
fn compile_and_eval(files: &[(String, String)]) -> Result<String, String> {
    ...
    let output = codegen_modules(&graph, &modules, &arena, &mut interner, &mut type_arena);
...
fn test_single_file_no_use() {
    let result = compile_and_eval(&[("main.gema".into(), "42".into())]);
...
fn test_typed_import_selects_correct_overload() {
    ...
    assert_eq!(
        result.as_deref(),
        Ok("4"),
        "typed import should select Num overload"
    );
}
